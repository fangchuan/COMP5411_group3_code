...
class Gaussian:
    ...
    #     # print(f"[GaussianModel] min scale: {scales.min().item()}, max scale: {scales.max().item()}, mean scale: {scales.mean().item()}")
    #     return scales
    @property
    def get_scaling(self):
        scales = self.scaling_activation(self._scaling).clamp(min=0.0, max=self.scaling_max)
        # print(f"[GaussianModel] min scale: {scales.min().item()}, max scale: {scales.max().item()}, mean scale: {scales.mean().item()}")
        return scales

    @property
    def get_rotation(self):
        return self.rotation_activation(self._rotation + self.rots_bias[None, :], p=2, dim=-1)
    ...
    def load_ply(self, path, transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]]):
        ...
        self._scaling = self.inverse_scaling_activation(scales)  # we set _scaling range to [-inf, inf], and GaussianModel will handle the activation and clamp
        self._rotation = rots - self.rots_bias[None, :]

    def to(self, device: torch.device = None, dtype: torch.dtype = None) -> "Gaussian":
        self._xyz = self._xyz.to(device, dtype)
        self._features_dc = self._features_dc.to(device, dtype)
        if self._features_rest is not None:
            self._features_rest = self._features_rest.to(device, dtype)
        self._opacity = self._opacity.to(device, dtype)
        self._scaling = self._scaling.to(device, dtype)
        self._rotation = self._rotation.to(device, dtype)
        return self
